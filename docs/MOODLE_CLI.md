...
Use `search-details` when an agent needs the searched page plus question text,
options, correction markers, and Moodle feedback/analysis in one read-only
call. Keep the page small; the command defaults to 20 and refuses values above
50. Use `--hide-correction` when answers and analysis should be hidden.

```bash
bin/moodle --profile dzexam --json \
