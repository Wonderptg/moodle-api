...
- OpenClaw: `plugins/moodle-aiagent-stack/openclaw.plugin.json`,
  `index.ts`, `index.js`, and `shared/openclaw-tools.mjs`
...
- `index.ts`: OpenClaw source entry using `definePluginEntry(...)`.
- `index.js`: OpenClaw installed runtime entry from
  `package.json#openclaw.runtimeExtensions`.
- `shared/openclaw-tools.mjs`: OpenClaw schemas and tool registration.
- `scripts/moodle-mcp-server.mjs`: MCP schemas and JSON-RPC framing.

OpenClaw 5.12 expects installed plugins to load JavaScript runtime entries
rather than relying on TypeScript source fallback.
